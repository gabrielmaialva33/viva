defmodule Viva.AI.PipelineTest do
  ...
  describe "process_thought/1" do
    test "processes spontaneous thought via MockClient" do
      ...
      # Setup Mock
      expect(Viva.AI.LLM.MockClient, :generate, fn ^prompt, _ ->
        {:ok, expected_thought}
      end)

      expect(Viva.Sessions.MockLifeProcess, :set_thought, fn ^avatar_id, ^expected_thought ->
        :ok
      end)
      ...
      # However, for this unit test example, I will assume we can't easily start the whole
      # RabbitMQ pipeline in isolation without more setup.
      ...
      # Ideally, we should export `process_thought/1` as public `process_message/1` for testing
      # or use @compile :export_all in test env.
      ...
      # or create a test that ensures `Application.get_env` is respected if I had a public entry.
      ...
      [_] = Viva.AI.Pipeline.handle_batch(:default, [message], :batch_info, :context)
