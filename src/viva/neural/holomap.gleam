...
pub type MapElitesGrid {
  MapElitesGrid(
    ...
    // Cells per dimension
    behavior_dims: Int,
    // Number of behavior dimensions (2 for 2D grid)
    min_bounds: List(Float),
    // Min values for each behavior dim
    max_bounds: List(Float),
    // Max values for each behavior dim
  )
}
...
pub type HoloMapConfig {
  HoloMapConfig(
    grid_size: Int,
    // Grid resolution (e.g., 20x20 = 400 cells)
    behavior_dims: Int,
    ...
    decay_midpoint: Int,
    // Generation at which weight is 50% decayed
    batch_size: Int,
...
pub fn qwen3_optimized_config() -> HoloMapConfig {
  HoloMapConfig(
    grid_size: 5,
    // 5x5 = 25 cells (Qwen3 recommendation)
    behavior_dims: 2,
    hrr_dim: 4096,
    initial_novelty_weight: 0.7,
...
pub fn tournament_select(
  ...
  case list.length(elites) {
    ...
    n -> {
      ...
      let participants =
        range_inclusive(0, tournament_size - 1)
        |> list.map(fn(i) {
...
pub fn normalize_hrr(vector: List(Float)) -> List(Float) {
  let magnitude =
    float_sqrt(list.fold(vector, 0.0, fn(acc, x) { acc +. x *. x }))
...
pub fn normalize_grid_hrr(grid: MapElitesGrid) -> MapElitesGrid {
  let normalized_cells =
    dict.map_values(grid.cells, fn(_key, elite) {
...
pub fn holographic_crossover(
  ...
  let blended =
    list.zip(parent1.hrr_vector, parent2.hrr_vector)
...
// Use viva_math/scalar (exact hardware exp, clamped against overflow;
// replaces the old 6-term Taylor approximation)
