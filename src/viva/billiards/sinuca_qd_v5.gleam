//// VIVA Sinuca - QD Trainer v5 (Impact-Based Descriptors)
////
//// Key fix: Behavior Descriptors now capture STRATEGY, not DEBRIS
//// - D1: first_contact_angle (normalized hit angle)
//// - D2: balls_scattered_ratio (movement caused)
////
//// Based on Gemini/Qwen3 analysis: BD must correlate with fitness
////
...
// =============================================================================
// IMPACT-BASED BEHAVIOR (correlates with fitness!)
// =============================================================================

pub type ImpactBehavior {
  ImpactBehavior(
    hit_angle: Float,        // Normalized angle of first contact (0-1)
    scatter_ratio: Float,    // Ratio of balls that moved (0-1)
    power_used: Float,       // Normalized power (0-1)
    combo_potential: Float,  // Estimated follow-up potential (0-1)
  )
}

fn behavior_to_cell(behavior: ImpactBehavior, grid_size: Int) -> #(Int, Int) {
  // Use hit_angle and scatter_ratio as primary dimensions
...
pub type QDConfig {
  QDConfig(
    grid_size: Int,           // 10x10 = 100 cells (bigger = more diversity)
    ...
    annealing_gens: Int,
    // New: Fitness threshold for exploitation boost
...
pub fn default_qd_config() -> QDConfig {
  QDConfig(
    grid_size: 10,           // 10x10 = 100 cells (was 5x5 = 25)
    ...
    annealing_gens: 50,
    fitness_boost_threshold: 30.0,  // When best > 30, boost exploitation
  )
}
...
fn annealed_novelty_weight(generation: Int, config: QDConfig, best: Float) -> Float {
  ...
  // If we have good fitness, reduce exploration further
...
// =============================================================================
// IMPACT-BASED EVALUATION (key difference from v4)
// =============================================================================

fn evaluate_genome(
  genome: Genome,
  config: QDConfig,
) -> #(Float, ImpactBehavior) {
  let table = sinuca.new()
  let episode = fitness.new_episode()

  // Track impact metrics across all shots
      ...
      fn(acc, _shot_num) {
        ...
        case sinuca.balls_on_table(current) <= 1 {
          ...
          False -> {
            ...
            // Remember positions before shot
            ...
            // Normalize shot angle to 0-1
            let normalized_angle = { shot.angle +. 3.14159 } /. { 2.0 *. 3.14159 }
            let clamped_angle = float_clamp(normalized_angle, 0.0, 1.0)

            let new_angles = [clamped_angle, ..angles]
            let new_scatters = [scatter_ratio, ..scatters]
            let new_pows = [shot.power, ..pows]
  ...
  // Combo potential: higher fitness = better follow-up
...
fn get_all_ball_positions(table: Table) -> List(#(Float, Float)) {
  // Get positions of all object balls (sinuca brasileira colors)
  list.filter_map(
...
fn calculate_scatter_ratio(before: List(#(Float, Float)), after: List(#(Float, Float))) -> Float {
  // Count how many balls moved significantly
  ...
  let moved = list.filter(pairs, fn(pair) {
    ...
    dist_sq >. 0.01  // Movement threshold: 0.1 units
  })
...
pub fn train(
  ...
  io.println("=== VIVA Sinuca QD v5 (Impact-Based BD) ===")
  io.println("GPU Status: " <> glands.check())
  io.println(
    "Grid: " <> int.to_string(qd_config.grid_size) <> "x"
    <> int.to_string(qd_config.grid_size)
    <> " (" <> int.to_string(qd_config.grid_size * qd_config.grid_size) <> " cells)"
  )
  io.println(
    "BD: [hit_angle, scatter_ratio] | Annealing: w=" <> float_str(qd_config.initial_novelty_weight)
...
fn train_loop(
  ...
  case remaining <= 0 {
    True -> {
      ...
      io.println("")
      io.println("=== QD v5 Training Complete ===")
      io.println("Best fitness: " <> float_str(best))
      io.println("Coverage: " <> float_str(cov) <> "%")
      io.println("QD-Score: " <> float_str(qd))
      #(grid, best, cov, qd)
