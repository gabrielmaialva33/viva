...
pub fn literature_baselines() -> List(LiteratureBaseline) {
  [
    LiteratureBaseline(
      ...
      cartpole_evals_sec: 1_200_000.0,
      // 1.2M evals/sec on A100
    ...
    LiteratureBaseline(
      ...
      cartpole_evals_sec: 2_000_000.0,
      // ~2M SPS
    ...
    LiteratureBaseline(
      ...
      cartpole_evals_sec: 5000.0,
      // Single-core Python
      pendulum_evals_sec: 3000.0,
      hardware: "Single CPU",
...
pub fn benchmark_cartpole(config: BenchConfig) -> BenchmarkMetrics {
  ...
  let mean_return = result_unwrap(statistics.mean(final_returns), 0.0)
  let std_return = result_unwrap(statistics.sample_stddev(final_returns), 0.0)
...
fn evolution_loop_cartpole(
  ...
  case remaining <= 0 {
    ...
    False -> {
      ...
      // Evolve
      let next_pop =
        neat.evolve(population, fitness_results, config, seed + remaining)
...
fn run_cartpole_episodes(
  ...
  case remaining <= 0 {
    ...
    False -> {
      ...
      let actions =
        list.map(range_inclusive(0, n - 1), fn(i) {
          let obs = [
...
fn print_banner() -> Nil {
  ...
  io.println(
    "              Academic-Grade Performance Comparison                     ",
  )
  io.println(
    "========================================================================",
  )
...
fn print_metrics(m: BenchmarkMetrics) -> Nil {
  ...
  io.println(
    "  Final return:     "
    <> float_str(m.final_return)
    <> " +/- "
    <> float_str(m.return_std),
  )
...
fn print_comparison_table(results: BenchmarkResults) -> Nil {
  ...
  io.println(
    "| Framework             | Evals/sec        | Hardware    | Speedup  |",
  )
  io.println(
    "+-----------------------+------------------+-------------+----------+",
  )
...
fn print_markdown_table(results: BenchmarkResults) -> Nil {
  ...
  io.println("")
  io.println(
    "| Environment | VIVA (RTX 4090) | EvoJAX (A100) | Brax (TPU) | NEAT-Python |",
  )
  io.println(
    "|-------------|-----------------|---------------|------------|-------------|",
  )
  ...
  io.println("")
  io.println("| Metric | CartPole | Pendulum |")
  io.println("|--------|----------|----------|")
  io.println(
    "| Final Return | "
...
fn warmup_loop(batch: cartpole.CartPoleBatch, remaining: Int) -> Nil {
  case remaining <= 0 {
    True -> Nil
    False -> {
      let actions = list.map(range_inclusive(0, 99), fn(_) { 0 })
...
fn result_unwrap_baseline(
  r: Result(LiteratureBaseline, e),
) -> LiteratureBaseline {
  case r {
    Ok(b) -> b
    Error(_) ->
